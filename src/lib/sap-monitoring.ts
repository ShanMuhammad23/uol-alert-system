...
import type { Student, GpaHistoryEntry } from "@/app/(home)/fetch";
import { THRESHOLDS } from "@/app/(home)/fetch";
...
function deriveAttendanceAlertLevel(percentage: number): "critical" | "warning" | null {
  if (percentage <= THRESHOLDS.attendance.critical) return "critical";
  if (percentage <= THRESHOLDS.attendance.warning) return "warning";
  return null;
}
...
export function mapMonitoringToStudents(entries: MonitoringEntry[]): Student[] {
  ...
  for (const e of entries) {
    ...
    const attLevel = deriveAttendanceAlertLevel(pct);
